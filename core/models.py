    ...
    def forward(self, data, step_ratio=1):
        ...
        for i in range(_b):
            ...
            for j in range(self.opt.num_input_views):                
                _fovy = np.rad2deg(2 * np.arctan(1/2*_intrin[i,j,1,1].item()))
                rays_o, rays_d = get_rays(cam_poses[i,j], self.opt.input_size, self.opt.input_size, _fovy)#self.opt.fovy) # [h, w, 3]
                rays_plucker = torch.cat([torch.cross(rays_o, rays_d, dim=-1), rays_d], dim=-1) # [h, w, 6]
                rays_embeddings.append(rays_plucker)
        ...
        # proj_mat[0, 0] = 1 / _fovy50
        # proj_mat[1, 1] = 1 / _fovy
        ...
        proj_matrices = torch.zeros((_b, _v, 4, 4),dtype=torch.float16, device=images.device)
        for i in range(_b):
            for j in range(_v):
                _fy = 1/2*_intrin[i,j,1,1].item()
                _fx = 1/2*_intrin[i,j,0,0].item()
                proj_mat = torch.zeros((4, 4), dtype=torch.float16, device=images.device)
                proj_mat[0, 0] = 1 / _fx
                proj_mat[1, 1] = 1 / _fy
                proj_mat[2, 2] = (self.opt.zfar + self.opt.znear) / (self.opt.zfar - self.opt.znear)
                proj_mat[3, 2] = - (self.opt.zfar * self.opt.znear) / (self.opt.zfar - self.opt.znear)
                proj_mat[2, 3] = 1
                proj_matrices[i,j] = proj_mat
